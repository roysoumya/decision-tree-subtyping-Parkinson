train_data_read = read.csv(file= "train_data_imputed_ML_classification.csv", header = TRUE, stringsAsFactors = FALSE)
test_data_read = read.csv(file= "test_data_imputed_ML_classification.csv", header = TRUE, stringsAsFactors = FALSE)

full_data_read = rbind(train_data_read, test_data_read)
write.csv(full_data_read, file= "total_data_clustering.csv", row.names = FALSE)

library(dplyr)

# read clustering data with assignments
cluster_data = read.csv(file = "total_data_clustering_with_assignments.csv", header = TRUE, stringsAsFactors = FALSE)
...
subtype_map_data = read.csv(file = "lrrk2_nonmotor_part1_analysis_subtypes.csv", header = TRUE, stringsAsFactors = FALSE)
...
write.csv(cluster_data1, file = "cluster_comp_final.csv", row.names = FALSE)

# create subset where subtype_id id not NA

cluster_data2 = subset(cluster_data1, is.na(subtype_id) ==FALSE )
write.csv(cluster_data2, file = "subset_cluster_comp_final.csv", row.names = FALSE)
